...
namespace Net_2kBot.Modules
{
    public static class Zuan
    {
        public static async void Execute(MessageReceiverBase @base)
        {
            ...
            if (receiver.MessageChain.Count <= 2) return;
            if (receiver.MessageChain[1].ToString() != "AtMessage { Type = At, Target = 2810482259 }" || receiver.MessageChain[2].ToString() != "PlainMessage { Type = Plain, Text =  你就是歌姬吧 }") return;
            string[] words =
            {
                "cnmd",
                "你更是歌姬吧嗷",
